...
template_filename = 'era5_example2_wotanh_tdim{}_slen{}_sslen{}_ns{}_bs{}_ne{}_dm{}_el{}_clipped_lr'.format(
    trunc_dim,
    selection_length,
...
for epoch in tqdm(range(num_epochs)):        
    ...
    for batch_idx in range(num_batches_per_epoch):
        ...
        batch_reconstr_vals, batch_grads, batch_rewards, best_sample_batch, batch_prob_hist = model_forward_jit(model.params, batch_sel_arrs)
        ...
        lr_mod = jnp.clip(lr_mod, 1e-5, 1e-1)
        ...
        # jax.debug.print("adv: {}",      adv)
        # jax.debug.print("min(abs(adv)): {}",  jnp.min(jnp.abs(adv)))
        # jax.debug.print("lr_mod: {}",    lr_mod)
        # jax.debug.print("grads norm: {}", jax.tree.map(lambda g: jnp.linalg.norm(g), total_batch_grads))
