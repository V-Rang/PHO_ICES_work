...
def _process_one_sample(params, apply_fn, X_hat_mod, lhs_mat, 
                        X_train, Y_train, phi_mat, A_hat, 
                        A_tilde_operator, U_r, library_functions, lam_vec, 
                        perms, selection_length, num_iters, sample_arr):
    
    # jax.debug.print("{}", sample_arr) # (m = multiple of 4)
    
    mask = sample_arr.astype(bool)
    idxs = jnp.nonzero(mask, size=nnz)[0]
    X_hat_nl = jnp.take(X_hat_mod, idxs, axis=0)

    # jax.debug.print("{}, {}", X_hat_mod.shape, X_hat_nl.shape) #(16, 1139), (8, 1139)
    ...
    # jax.debug.print("{}", sel_slices.shape)

    (final_grads, _), prob_hist = jax.lax.scan(_body, (init_grads, jnp.array([1.0], jnp.float32)), sel_slices) # iterate over elements of sel_slices

    # jax.debug.print("{}", prob_hist)
...
class Model:
    ...
    def __init__(self, model_settings: Dict[str, Any]):
        ...
        self.perms = jnp.array(model_settings['permutations']) # (6, 4)
    ...
    def forward(self, inp_params, inp_sel_arrs: np.ndarray) -> Tuple[Any, ...]:
        ...
        err_computation_fn = jax.vmap(_process_one_sample, 
                                        in_axes=(
                                                None,   # params
                                                None,   # apply_fn
                                                None,   # X_hat_mod
                                                ...
                                                None,   # phi_mat
                                                None,   # A_hat
                                                None,   # A_tilde_operator
                                                None,   # U_r
                                                None,   # library_functions  (a Python tuple; must be static)
                                                None,      # lam_vec            (vector of lambdas -> map over axis 0)
                                                None,      # perms              (B x selection_length) one perm per map
                                                None,   # selection_length
                                                None,   # num_iters
                                                0       # batch              (if this is an array of, say, batch ids)
                                            )
                                    )                                                    

        self.params = inp_params
        
        batch_reconstr_vals, batch_grads, batch_rewards, phi_bar_opt, prob_hist, lam_opt = err_computation_fn(self.params, self.apply_fn, self.X_hat_mod, self.lhs_mat, 
                                                                                                              self.X_train, self.Y_train, self.phi_mat, self.A_hat, 
                                                                                                              self.A_tilde_operator, self.U_r, self.library_functions, self.lam_vec, 
                                                                                                              self.perms, self.selection_length, num_iters, batch)
