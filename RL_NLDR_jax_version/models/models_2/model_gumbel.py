...
# def reconstruction_error_2(X_train_t_batch, Y_train_t_batch, A_hat, 
#                         H_hat, U_r, X_hat_nl_t_batch):
...
class ROMModel(nn.Module):
    ...
    def __call__(self, X_hat_t_batch, X_train_t_batch, Y_train_t_batch, temp):
        ...
        # X_pred_batch, reconstr_err = reconstruction_error_2(X_train_t_batch, Y_train_t_batch, self.A_hat, 
        #                                     H_hat, self.U_r, X_hat_nl_t_batch)
...
def train_for_epochs(state, library_functions, phi_mat, A_tilde, U_r, A_hat, X_hat, X_train, Y_train, X_val, Y_val, num_epochs, start_temp, batch_size, rng, initial_lr, threshold):
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        # selected_indices = np.asarray(np.argmax(logits_cpu, axis=1), dtype=np.int32)  # shape (p_val,)
