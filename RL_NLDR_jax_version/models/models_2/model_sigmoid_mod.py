...
def get_prob_and_grad(params, apply_fn, perms, sample_arr) -> Tuple[jnp.ndarray, Dict[str, Any]]:
    ...
    def one_pair(c, n):
        ...
        return (grad0, p0)
    sample_grads, batch_probs = jax.vmap(one_pair, in_axes=(0, 0))(cur, nxt)
    summed_grads = jax.tree.map(lambda g: jnp.sum(g, axis=0), sample_grads)
    return (summed_grads, batch_probs)
...
class Model:
    ...
    def forward(self, inp_params, batch):
        ...
        batch_grads, batch_prob_hist = grad_computation_fn(inp_params, self.apply_fn, self.perms, batch)
