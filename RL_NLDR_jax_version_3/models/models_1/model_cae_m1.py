...
from utils.tools_1 import make_rom_reconstruction_error
from layers.output_grad_comp import output_selection
from utils.tools_1 import make_library_functions, build_library
import math
import pandas as pd
...
def validation_loss_compute(S_val: jnp.array, 
    ...
    def step(xh, _):
        ...
        mod_sel = jnp.take(xh, selected_indices_spt)

        xh = A_hat @ xh + H_hat_opt @ mod_sel
        return (xh, xh)
...
class ConcreteAutoencoder(nn.Module):
    ...
    def setup(self):
        ...
        self.rom_err_fn = make_rom_reconstruction_error(self.phi_mat, self.A_hat, self.A_tilde, self.U_r, self.library_functions)

    def __call__(self, X_batch_t, Y_batch_t, lhs_mat_batch_t, temperature_spt): # x: rval

        X_hat_batch_t = X_batch_t @ self.U_r

        X_hat_mod_batch_t = build_library(X_hat_batch_t, self.library_functions)  # (k, r x len(lib))

        X_hat_nl_batch_t, temperature_spt, selected_idx, l0_pen = self.encoder(X_hat_mod_batch_t, temperature_spt) # (k, p)
...
def train_for_epochs(state, X_train, Y_train, lhs_mat, S_val, phi_mat, A_tilde, U_r, library_functions, A_hat, num_epochs, start_temp, batch_size, rng, initial_lr, threshold):
