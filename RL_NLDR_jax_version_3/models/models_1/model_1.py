...
class ConcreteAutoencoder(nn.Module):
    ...
    def setup(self):
        self.selector = ConcreteSelector(self.start_temp, self.min_temp, self.alpha_const, input_dim=self.input_dim, output_dim=self.output_dim)

    def __call__(self, x, x_mat_t_batch, y_mat_t_batch, lhs_mat_t_batch, temperature):
        x_hat_nl_t_batch, temperature, selected_indices = self.selector(x, temperature)

        def _compute_err(lam):
            phi_bar = lstsq_l2(x_hat_nl_t_batch, lhs_mat_t_batch, lam).T
            return rom_reconstruction_error(x_mat_t_batch, y_mat_t_batch, self.phi_mat, self.A_hat, self.A_tilde, phi_bar, self.U_r, self.library_functions, selected_indices)
        errs = jax.vmap(_compute_err)(self.lam_vec)
        i_opt = jnp.nanargmin(errs)
        err_opt = errs[i_opt]
        return (err_opt, temperature)

def create_train_state(rng, model, input_shape, full_dim, l_val_dim, lr):
    rng_params, rng_gumbel, rng_Dropout = jax.random.split(rng, num=3)
    init_vars = model.init({'params': rng_params, 'gumbel': rng_gumbel, 'dropout': rng_Dropout}, jnp.empty(input_shape), jnp.empty((input_shape[0], full_dim)), jnp.empty((input_shape[0], full_dim)), jnp.empty((input_shape[0], l_val_dim)), 1.0)
    params = init_vars['params']
    tx = optax.adam(lr, b1=0.9, b2=0.999, eps=1e-07, eps_root=False)
    return TrainState.create(apply_fn=model.apply, params=params, tx=tx)

@jax.jit
def train_step(state, x_hat_mod_t_batch, x_mat_t_batch, y_mat_t_batch, lhs_mat_t_batch, temperature, rngs):

    def loss_fn(params):
        loss, new_temperature = state.apply_fn({'params': params}, x_hat_mod_t_batch, x_mat_t_batch, y_mat_t_batch, lhs_mat_t_batch, temperature, rngs={'gumbel': rngs['gumbel'], 'dropout': rngs['dropout']})
...
def train_for_epochs(state, X_hat_mod_t, X_mat_t, Y_mat_t, lhs_mat_t, num_epochs, start_temp, batch_size, rng):
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        for start in starts:
            ...
            Y_mat_t_batch = Y_mat_t[start:start + batch_size]
            X_mat_t_batch = X_mat_t[start:start + batch_size]
            lhs_mat_t_batch = lhs_mat_t[start:start + batch_size]
            rng, rng_g, rng_d = jax.random.split(rng, 3)
            state, batch_loss, temperature = train_step(state, X_hat_mod_t_batch, X_mat_t_batch, Y_mat_t_batch, lhs_mat_t_batch, temperature, rngs={'gumbel': rng_g, 'dropout': rng_d})
