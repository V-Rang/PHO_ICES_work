...
from utils.tools_1_normalized_m2 import apply_selected_funcs, lstsq_l2
from utils.tools_1_normalized_m2 import make_rom_reconstruction_error
from layers.output_grad_comp import output_selection
from utils.tools_1_normalized_m2 import make_library_functions, build_library
import math
import pandas as pd
...
def validation_loss_compute(S_val: jnp.array, 
    ...
    # jax.debug.print("{}:{}", rhs_term2.min(), rhs_term2.max()) # -0.09018245339393616:0.13742975890636444
    # jax.debug.print("{}:{}", phi_bar_opt.min(), phi_bar_opt.max()) # -44.12687301635742:49.31719207763672
    # jax.debug.print("{}:{}", A_tilde.min(), A_tilde.max()) # -0.1043911799788475:0.9998041987419128

    def step_scaled(alpha):
        def step(xh, _):
            mod = apply_selected_funcs(xh, library_functions)                  # (m,)
            # mod_sel = jnp.take(mod, selected_indices_spt, mode='raise')        # (K,)
            mod_sel = jnp.take(mod, selected_indices_spt)
            # jax.debug.print("{}:{}", mod_sel.min(), mod_sel.max())

            mask = jnp.isfinite(mod_sel)
            bad_count = jnp.size(mod_sel) - jnp.sum(mask)
            mod_safe = jnp.where(mask, mod_sel, 0.0)

            rhs1 = A_tilde @ xh
            rhs2 = rhs_term2

            rhs3_safe = (phi_bar_opt @ (alpha * mod_safe))

            any_bad = bad_count > 0

            # xh_next = rhs1 + rhs2 + rhs3_safe

            xh_next = rhs1 + rhs2

            # Debug prints
            # jax.debug.print(
            #     "a={:.2e} | bad_feats={} | ||mod_sel||={:.3e} ||rhs1||={:.3e} ||rhs3_safe||={:.3e}",
            #     alpha, bad_count, jnp.linalg.norm(mod_safe), jnp.linalg.norm(rhs1), jnp.linalg.norm(rhs3_safe)
            # )

            jax.debug.print(
                "a={:.2e} | bad_feats={} | ||mod_sel||={:.3e} ||rhs1||={:.3e}",
                alpha, bad_count, jnp.linalg.norm(mod_safe), jnp.linalg.norm(rhs1)
            )

            # return xh_next, jnp.linalg.norm(rhs3_safe)
            return xh_next, _
    ...
    for alpha in [1e-2]:
        # _, rhs3_norms = jax.lax.scan(step_scaled(alpha), x_tilde0_normalized, None, length=S_val.shape[1])
        # jax.debug.print("alpha={} | rhs3 norms: min={:.3e} med={:.3e} max={:.3e}",
        #                 alpha, jnp.min(rhs3_norms), jnp.median(rhs3_norms), jnp.max(rhs3_norms))

        _, _ = jax.lax.scan(step_scaled(alpha), x_tilde0_normalized, None, length=S_val.shape[1])
        # jax.debug.print("alpha={} | rhs3 norms: min={:.3e} med={:.3e} max={:.3e}",
        #                 alpha, jnp.min(rhs3_norms), jnp.median(rhs3_norms), jnp.max(rhs3_norms))
    ...
    def step(xh, _):
        ...
        # jax.debug.print("{}:{}", xh.min(), xh.max()) 
        # 0.16993701457977295:0.6215534806251526
        # -7.007695198059082:5.3065056800842285
        # -9068.822265625:7668.9501953125
        # -3588734549229568.0:4366734320467968.0

        mod = apply_selected_funcs(xh, library_functions) # e.g. shape (m,)
        mod_sel = jnp.take(mod, selected_indices_spt)      # (K,)

        # jax.debug.print("{}", selected_indices_spt)
        # jax.debug.print("{}:{}", mod.min(), mod.max()) # -44.12687301635742:49.31719207763672
        # jax.debug.print("{}:{}", mod_sel.min(), mod_sel.max()) # -44.12687301635742:49.31719207763672

        #***************************************************
        # finite_mask = jnp.isfinite(mod_sel)
        # any_bad = ~jnp.all(finite_mask)

        # # Safe stats (avoid touching NaNs/Infs)
        # mod_safe = jnp.where(finite_mask, mod_sel, 0.0)

        # # jax.debug.print("{}:{}", mod_sel.min(), mod_sel.max()) # -44.12687301635742:49.31719207763672
        # # jax.debug.print("{}:{}", type(mod_sel), mod_sel.dtype) # <class 'jaxlib.xla_extension.ArrayImpl'>:float32
        
        # safe_min = jnp.where(jnp.any(finite_mask),
        #                     jnp.min(jnp.where(finite_mask, mod_sel, jnp.inf)),
        #                     jnp.nan)

        # # jax.debug.print("{}:{}:{}", mod_sel.min(), mod_sel.max(), safe_min) # -44.12687301635742:49.31719207763672
        # # jax.debug.print("{}", safe_min)

        # safe_max = jnp.where(jnp.any(finite_mask),
        #                     jnp.max(jnp.where(finite_mask, mod_sel, -jnp.inf)),
        #                     jnp.nan)

        # safe_norm = jnp.linalg.norm(mod_safe)

        # # Find a representative bad index (if any)
        # bad_idx = jnp.where(~finite_mask, size=1, fill_value=-1)[0][0]
        # bad_val = jnp.where(bad_idx >= 0, mod_sel[bad_idx], jnp.nan)

        # # Top contributor computed from safe values (so printing won't crash)
        # contrib = jnp.abs(phi_bar_opt) * jnp.abs(mod_safe)
        # top_idx = jnp.argmax(contrib)
        #***************************************************

        # jax.debug.print(
        #     "any_bad={} bad_idx={} bad_val={:.3e} | mod min/max={:.3e}/{:.3e} ||mod||={:.3e} | top_feat={} val={:.3e} w={:.3e} contrib={:.3e}",
        #     any_bad, bad_idx, bad_val,
        #     safe_min, safe_max, safe_norm,
        #     top_idx, mod_sel[top_idx], phi_bar_opt[top_idx], contrib[top_idx]
        # )
        ...
        rhs_term3 = phi_bar_opt @ mod_sel

        # jax.debug.print(
        #     "||xh||={:.3e}, ||rhs1||={:.3e}, ||rhs2||={:.3e}, ||rhs3||={:.3e}, "
        #     "finite={}",
        #     jnp.linalg.norm(xh), jnp.linalg.norm(rhs_term1), jnp.linalg.norm(rhs_term2),
        #     jnp.linalg.norm(rhs_term3),
        #     jnp.all(jnp.isfinite(jnp.array([xh, mod_sel, rhs_term1, rhs_term3]))),
        # )
    ...
    _, xh_seq = jax.lax.scan(step, x_tilde0_normalized, None, length=S_val.shape[1])

    X_tilde_rec_batch = (max_tilde - min_tilde) * xh_seq.T + min_tilde
    X_rec_batch = U_l @ X_tilde_rec_batch

    num = jnp.linalg.norm(S_val - X_rec_batch)
    den = jnp.linalg.norm(S_val) + 1e-12       # small eps for safety
    return num / den
...
def train_step(state, X_batch_t, Y_batch_t, lhs_mat_batch_t, temperature_spatial, rngs):
    ...
    # jax.debug.print("{}", grads['encoder']['logits'][:3, :3]) # all nans

    # jax.debug.print("{}:{}", grads['encoder']['logits'].min(), grads['encoder']['logits'].max()) # all nans

    # jax.debug.print("{}", batch_loss) # 0.1914520263671875 to start.
    # jax.debug.print("{}", grads['encoder']['logits'][:5, :5]) # all nans
...
class ConcreteAutoencoder(nn.Module):
    ...
    start_temp: float
    alpha_const: float
    enc_inp_dim: int # lval x len(lib)
    ...
    def setup(self):
        ...
        self.rom_err_fn = make_rom_reconstruction_error(self.A_tilde, self.U_l, self.min_tilde, self.max_tilde, self.library_functions)
    ...
    def __call__(self, X_batch_t, Y_batch_t, lhs_mat_batch_t, temperature_spt): # x: rval
        ...
        X_tilde_mod_batch_normalized_t = build_library(X_tilde_batch_normalized_t, self.library_functions)  # (k, r x len(lib))

        X_tilde_nl_batch_normalized_t, temperature_spt, selected_idx, soft_selection = self.encoder(X_tilde_mod_batch_normalized_t, temperature_spt) # (k, p)
        ...
        # jax.debug.print("{}:{}", lhs_mat_batch_t.min(), lhs_mat_batch_t.max())   # -0.014036864042282104:0.017253777012228966
        # jax.debug.print("{}:{}", X_tilde_nl_batch_normalized_t.min(), X_tilde_nl_batch_normalized_t.max())   # 0.19693948328495026:0.20773765444755554

        def _compute_err(lam):
            phi_bar = lstsq_l2(X_tilde_nl_batch_normalized_t, lhs_mat_batch_t, lam).T # lhs_mat_batch_t : (k, l)
            # return self.rom_err_fn(X_batch_t, Y_batch_t, phi_bar, selected_idx)
            return self.rom_err_fn(X_batch_t, Y_batch_t, phi_bar, soft_selection) # soft_selection: (nf, D)
        ...
        # jax.debug.print("{}", err_opt) # 0.1914520263671875

        # jax.debug.print("{}:{}", X_tilde_nl_batch_normalized_t.min(), X_tilde_nl_batch_normalized_t.max()) # 0.19693948328495026:0.20773765444755554, goes to nans after first print.
        # jax.debug.print("{}:{}", lhs_mat_batch_t.min(), lhs_mat_batch_t.max()) # -0.014036864042282104:0.017253777012228966, doesn't go to nans.

        phi_bar_opt = lstsq_l2(X_tilde_nl_batch_normalized_t, lhs_mat_batch_t, lam_opt).T

        # init_grads = jax.tree.map(lambda p: jnp.zeros_like(p), params)

        # jax.debug.print("{}:{}", phi_bar_opt.min(), phi_bar_opt.max()) # -0.001662622787989676:0.0019091545837000012
...
def train_for_epochs(state, X_train, Y_train, lhs_mat, S_val, A_tilde, U_l, min_tilde, max_tilde, library_functions, num_epochs, start_temp, batch_size, rng, initial_lr, threshold):
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        for start in starts:
            ...
            # print(temperature_spt)
            # print(grad_vals['encoder']['logits'][:5, :5])
