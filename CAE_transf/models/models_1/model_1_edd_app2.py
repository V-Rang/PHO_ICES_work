...
def reconstr_loss(gt, reconstr):
    frob = jnp.linalg.norm(reconstr - gt)
    d = gt.shape[1]
    return frob / d
...
class Network_Decoder(nn.Module):
    ...
    def __call__(self, x, deterministic: bool=False):
        ...
        return x
decoder_input_dim = 22701
decoder_module_tester = Network_Decoder(input_dim=decoder_input_dim)

@jax.jit
def eval_loss(params, features, targets):
    preds = decoder_module_tester.apply({'params': params}, features, deterministic=True, mutable=False)
    return reconstr_loss(preds, targets)
...
class ConcreteAutoencoder(nn.Module):
    ...
    def setup(self):
        self.selector = ConcreteSelector(self.start_temp, self.min_temp, self.alpha_const, input_dim=self.input_dim, output_dim=self.encode_dim)
        self.decoder = Network_Decoder(self.input_dim)

    def __call__(self, x, temperature):
        svd_trunc_selection_array, temperature, selected_indices = self.selector(x, temperature)
        svd_trunc_selection_array_reconstr = self.decoder(svd_trunc_selection_array)
        err = reconstr_loss(x, svd_trunc_selection_array_reconstr)
        return (svd_trunc_selection_array_reconstr, err, temperature)

def create_train_state(rng, model, input_shape, lr):
    rng_params, rng_gumbel, rng_Dropout = jax.random.split(rng, num=3)
    init_vars = model.init({'params': rng_params, 'gumbel': rng_gumbel, 'dropout': rng_Dropout}, jnp.empty(input_shape), 1.0)
    params = init_vars['params']
    tx = optax.adam(lr, b1=0.9, b2=0.999, eps=1e-07, eps_root=False)
    return TrainState.create(apply_fn=model.apply, params=params, tx=tx)

@jax.jit
def train_step(state, svd_trunc_array, temperature, rngs):

    def loss_fn(params):
        _, loss, new_temperature = state.apply_fn({'params': params}, svd_trunc_array, temperature, rngs={'gumbel': rngs['gumbel'], 'dropout': rngs['dropout']})
...
def train_for_epochs(state, svd_trunc_train_array_t, svd_trunc_val_array_t, num_epochs, start_temp, batch_size, rng, initial_lr, threshold):
    """Runs the inner training loop and returns updated state + final loss history."""
    n_timesteps = svd_trunc_train_array_t.shape[0]
    train_loss_history = []
    val_loss_history = []
    mean_max_prob = []
    logit_vals_hist = []
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        starts = make_epoch_starts(subkey, svd_trunc_train_array_t.shape[0], batch_size, non_overlapping=True)
        epoch_loss = 0.0
        for start in starts:
            svd_trunc_train_array_batch = svd_trunc_train_array_t[start:start + batch_size]
            rng, rng_g, rng_d = jax.random.split(rng, 3)
            state, batch_loss, temperature = train_step(state, svd_trunc_train_array_batch, temperature, rngs={'gumbel': rng_g, 'dropout': rng_d})
            ...
            epoch_loss += batch_loss.item() * svd_trunc_train_array_batch.shape[0]
        ...
        selected_features = jnp.take(svd_trunc_val_array_t, selected_indices, axis=1)
        decoder_params = state.params['decoder']
        val_loss = eval_loss(decoder_params, selected_features, svd_trunc_val_array_t)
    ...
    return (state, temperature, train_loss_history, val_loss_history, mean_max_prob, logit_vals_hist, rng)