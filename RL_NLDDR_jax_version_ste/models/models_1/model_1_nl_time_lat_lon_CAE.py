...
def create_train_state(rng, model, input_shape, lr):
    ...
    init_vars = model.init({'params': rng_params, 'gumbel': rng_gumbel, 'dropout': rng_Dropout}, dummy_x, dummy_temperature, dummy_time)
...
def train_step(state, batch_inp, temperature, batch_time_vals, rngs):
    ...
    return (state, batch_decoder_output, batch_loss, new_temperature, grads)
...
encoder_network_module_tester = ConcreteSelector(start_temp = start_temp, 
                                                min_temp = min_temp, 
...
def validation_loss_compute(S_val_t_batch: jnp.array, 
                            val_times_batch, global_coords, selected_indices,  
                            state, temperature, batch_size):

    selected_coords = global_coords[selected_indices]

    dff_params = state.params['dff_network']
    decoder_params = state.params['decoder']
    encoder_params = state.params['encoder']

    # init_key, gumbel_key = jax.random.split(jax.random.PRNGKey(0), 2)
    
    # enc_output, temperature, selected_indices = self.encoder(x, temperature)
    # selected_coords = self.global_coords[selected_indices]
    # coords_expanded = jnp.broadcast_to(selected_coords[None, ...], (x.shape[0], self.enc_out_dim, 2))
    # time_expanded = jnp.broadcast_to(time_vals[:, None, :], (x.shape[0], self.enc_out_dim, 4))
    ...
    batch_enc_out, temperature, _ = encoder_network_module_tester.apply(
    ...
    coords_expanded = jnp.broadcast_to(selected_coords[None, ...], (S_val_t_batch.shape[0], encoder_output_dim, 2))
    val_time_expanded = jnp.broadcast_to(val_times_batch[:, None, :], (S_val_t_batch.shape[0], encoder_output_dim, 4))
    batch_dff_input = jnp.concatenate([val_time_expanded, coords_expanded], axis=-1)
    ...
    batch_loss = batch_loss_compute(S_val_t_batch, batch_preds, batch_dff_out, batch_enc_out, frac_dynamics)
...
# val_loss = eval_loss_batch(dff_params, decoder_params, val_dff_input, S_val_t)

# @jax.jit
# def eval_loss_batch(dff_params, decoder_params, dff_inp, targets):
...
def eval_loss_over_dataset(dff_params, decoder_params,  all_dff_inp, all_targets, batch_size=128):
...
class ConcreteAutoencoder(nn.Module):
    ...
    def __call__(self, x, temperature, time_vals):
        ...
        dff_output = dff_output[..., 0]
        decoder_output = self.decoder(dff_output)
        batch_loss = batch_loss_compute(x, decoder_output, dff_output, enc_output, omega_h=self.frac_dynamics)
...
def train_for_epochs(state, global_coords, trunc_dim, S_train, train_times, S_val, val_times, num_epochs, start_temp, batch_size, rng, initial_lr, threshold):
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        for start in starts:
            ...
            # S_val_t_batch = S_val_t[start:start + batch_size]
            # val_times_batch = val_times_enc[start:start + batch_size]

            rng, rng_g, rng_d = jax.random.split(rng, 3)
            
            state, batch_decoder_output, batch_loss, temperature, grad_vals = train_step(
            state, 
            S_train_t_batch, 
            temperature, 
            train_times_batch, 
            ...
            # batch_val_loss = validation_loss_compute(
            #     S_val_t_batch,
            #     val_times_batch,
            #     global_coords,
            #     selected_indices,
            ...
            # val_loss += batch_val_loss.item() * S_val_t_batch.shape[0]
        ...
        # def validation_loss_compute(S_val_t: jnp.array, 
        #                             val_times_expanded, global_coords, selected_indices, trunc_dim,  
        #                             state, temperature, batch_size):

        # val_loss_cpu = validation_loss_compute(
        #     S_val_t,
        #     val_times_expanded,
        #     global_coords,
        #     selected_indices,
    ...
    # print(state.params.keys()) # dict_keys(['decoder', 'dff_network', 'encoder'])

    state = state.replace(params=best_params)

    return (state, temperature, train_loss_history, val_loss_history, mean_max_prob, logit_vals_hist, best_overall_train_loss, best_overall_val_loss, preserved_grads)