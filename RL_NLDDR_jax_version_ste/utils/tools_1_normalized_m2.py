...
def make_rom_reconstruction_error(A_tilde, U_l, min_tilde, max_tilde, library_functions):
    ...
    @jax.jit
    def rom_reconstruction_error(X_batch_t, Y_batch_t, phi_bar, soft_selection):
        ...
        soft_selection: (nf, D)   soft selection array

        """
        

        # jax.debug.print("{}:{}", phi_bar.min(), phi_bar.max()) # not nans initially.

        soft_selection_matrix = soft_selection.T # (D, nf)
        ...
        def step(xh, _):
            ...
            mod_sel = mod @ soft_selection_matrix # (D, ) @ (D, nf) = (nf,)
        ...
        # jax.debug.print("hello {}:{}", xh_seq.min(), xh_seq.max()) # not nans

        X_tilde_rec_batch = (max_tilde - min_tilde) * xh_seq.T + min_tilde

        X_rec_batch = U_l @ X_tilde_rec_batch

        # jax.debug.print("{}:{}", X_rec_batch.min(), X_rec_batch.max()) # not nans
