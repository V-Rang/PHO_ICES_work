...
# print(S_train.shape, S_test.shape, S_val.shape) # (22701, 1657) (22701, 868) (22701, 988)
# (1657, 161, 141) (868, 161, 141) (988, 161, 141)

lat_vals = loaded_data['lat_vals']
lon_vals = loaded_data['lon_vals']

S_train_org = S_train.reshape((len(lat_vals), len(lon_vals), -1))
S_test_org = S_test.reshape((len(lat_vals), len(lon_vals), -1))
S_val_org = S_val.reshape((len(lat_vals), len(lon_vals), -1))

# print(S_train_org.shape, S_test_org.shape, S_val_org.shape) # (161, 141, 1657) (161, 141, 868) (161, 141, 988)
...
# results_dir = f'/workspace/venu_files/climate_forecasting/RL_NLDR_jax_version_3/results/var_red1_ne{num_epochs}_lib{len(library_functions)}_bs{batch_size}_lr{lr}_Nval{Nh_val}_lval{l_val}_rc{repulsion_coeff}_ste_m1_CNN/'
...
#     # jax.debug.print("{}:{} | {}:{} | {}:{} | {}:{}",linear_term.min(), linear_term.max(), c_tilde.min(), c_tilde.max(), phi.min(), phi.max(), x_next.min(), x_next.max())
